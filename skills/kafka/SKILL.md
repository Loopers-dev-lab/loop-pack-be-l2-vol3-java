...
**구현·적용 로드맵**(Spring 내부 이벤트 경계, `commerce-api` → Kafka → `commerce-collector`, 선착순 쿠폰, 토픽 예시, **`event_handled` vs 감사/로그 테이블 분리 이유**)은 **`KAFKA_APPLICATION.md` §0** 에 정리되어 있다. 리뷰 시 설계 의도가 문서와 맞는지 교차 확인한다.
...
- **멱등 처리 테이블(`event_handled` 등)과 감사·append-only 로그 테이블이 분리**되어 있는지
  - 동일 테이블에 합치면 멱등 정리(삭제/덮어쓰기)와 감사 보존 요구가 충돌할 수 있음 — 상세는 `KAFKA_APPLICATION.md` §0.5
