...
description:
  ...
  - 트랜잭션·네이밍·Repository 설계 등 코드·설계 일관성(리뷰 관점) 체크리스트
...
- **트랜잭션**
  - 클래스 레벨 `@Transactional` 사용 여부 → 사용하지 않을 것. 메서드 단위만, 필요한 메서드에만 선언.
- **readOnly**
  - Facade/Service 전체를 readOnly 기본값으로 두지 않았는지. 조회 전용 메서드에만 `@Transactional(readOnly = true)` 적용했는지.
- **네이밍**
  - 도메인/유즈케이스 메서드: `update`, `save` 대신 `updateBrandInfo` 등 구체적 동사 사용 여부.
  - 컴포넌트: `*Service`, `*Facade`만 사용했는지. `*ApplicationService`, `*DomainService` 등 레이어 접미사는 사용하지 않을 것.
- **불필요 호출**
  - 등록/수정 플로우에서 필요한 검증·조회만 호출했는지 (예: register 시 불필요한 getBrand 등 제거).
- **요구사항 범위**
  - `delete`, `deleteAllByBrandId` 등 요구사항/유즈케이스에 명시된 기능만 구현했는지.
- **Repository 일관성**
  - Repository는 애그리거트 루트 기준만 두었는지 (예: Order만, OrderItemRepository 없음).