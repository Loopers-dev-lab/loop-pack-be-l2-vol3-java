...
LOGIN_ID_PREFIX="${LOGIN_ID_PREFIX:-abca}"
...
register_user() {
  local login_id="$1"
  local code
  code=$(curl -sS -o /tmp/admission_strategy_register.json -w "%{http_code}" \
  ...
  if [[ "$code" != "201" && "$code" != "409" ]]; then
    ...
    cat /tmp/admission_strategy_register.json >&2
    exit 1
  fi
}
...
enter_queue() {
  local login_id="$1"
  local code
  code=$(curl -sS -o /tmp/admission_strategy_enter.json -w "%{http_code}" \
  ...
  if [[ "$code" != "201" ]]; then
    ...
    cat /tmp/admission_strategy_enter.json >&2
    exit 1
  fi
}
...
for i in $(seq 1 "${USER_COUNT}"); do
  ...
  register_user "$login_id"
  enter_queue "$login_id"
done

printf 'prepared_users=%s prefix=%s\n' "$USER_COUNT" "$LOGIN_ID_PREFIX"
