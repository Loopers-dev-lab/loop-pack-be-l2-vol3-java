...
계층 순서대로 구현하되, **2개 체크포인트**에서 사용자에게 요약을 공유한다.

### 구현 순서 + 체크포인트

```
1. domain    → Entity, Repository 인터페이스  ─┐
2. infra     → RepositoryImpl, JpaRepository  ─┘→ [체크포인트 1: 데이터 구조]
3. application → Service, Facade, Command, Info ─┐
4. interfaces → Controller, ApiSpec, Request, V1Dto ─┘→ [체크포인트 2: 비즈니스 흐름]
```

### 체크포인트 요약 포맷

각 체크포인트에서 사용자에게 10초 안에 스캔 가능한 요약을 공유한다:

```
## 체크포인트 {N}/2: {데이터 구조 | 비즈니스 흐름} 완료

| 파일 | 라인 | 핵심 |
|------|------|------|
| {파일명} | {N}줄 | {역할 — 핵심 멤버/메서드 한 줄} |

총 {N}줄. {위험도 판정}.
{위험도에 따른 안내}
```

### Ship/Show/Ask 위험도 자동 분류

체크포인트마다 위험도를 판정하여 리뷰 강도를 조절한다:

| 위험도 | 조건 | 체크포인트 동작 |
|--------|------|---------------|
| **Ship** | 기존 패턴 그대로 + 총 150줄 이하 | 요약만 보여주고 자동 진행 ("기존 패턴과 동일합니다. 계속 진행합니다.") |
| **Show** | 기존 패턴 + 200줄 초과, 또는 약간의 변형 | 요약 보여주고 승인 대기 ("계속 진행할까요?") — **기본값** |
| **Ask** | 새 패턴, 외부 통합, 동시성 제어, 복잡한 비즈니스 규칙 | 변경 파일별 핵심 로직 상세 설명 + 승인 대기 ("아래 내용을 확인해주세요.") |

위험도 판단 기준:
- Phase 2 SCAN에서 참고한 도메인이 있고, 같은 패턴이면 → Ship 또는 Show
- 참고 도메인이 없거나(첫 도메인), spec에 외부 통합/동시성이 포함되면 → Ask
- 200줄은 SmartBear/Cisco 연구 기반 (200줄 초과 시 리뷰 품질 급락)
...
```
1. 단위 테스트   → Entity 불변식, 검증 규칙  ─→ [체크포인트 3: 단위 테스트]
2. 통합 테스트   → Service + DB (중복 체크, 존재 확인) ─┐
3. E2E 테스트   → HTTP 전체 흐름                    ─┘→ [체크포인트 4: 통합+E2E 테스트]
```

체크포인트 요약은 Phase 3과 동일한 포맷. 단위 테스트는 순수 로직이므로 Ship 확률 높음.
...
REPORT 출력 후, 사용자에게 확인한다:

```
구현이 완료되었습니다. 바로 병렬 검증(/parallel-review)을 실행할까요?
```

사용자가 승인하면 `/parallel-review {epic}/{feature}`를 실행한다.
사용자가 거부하면 나중에 수동으로 실행할 수 있음을 안내한다.
