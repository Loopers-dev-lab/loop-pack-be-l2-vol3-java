...
/**
 * 월간 랭킹 API E2E — HTTP → Controller → Facade → DB (MV 테이블).
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@DisplayName("MonthlyRankingV1 API E2E")
class MonthlyRankingV1ApiE2ETest {
    ...
    private void seedMonthlyRanking(Long productId, LocalDate baseDate, int rank, double score) {
        jdbcTemplate.update(
    ...
    class GetMonthlyRanking {
        ...
        @Test
        @DisplayName("200 — 상품 정보가 Aggregation 된 월간 랭킹 Page 를 반환한다.")
        void happyPath() {
            // given
            ...
            seedMonthlyRanking(p1.getId(), baseDate, 1, 10.0);
            seedMonthlyRanking(p2.getId(), baseDate, 2, 5.0);
        ...
        void visibilityFilter() {
            ...
            LocalDate baseDate = LocalDate.now(clock).minusDays(1);
            seedMonthlyRanking(hidden.getId(), baseDate, 1, 20.0);
            seedMonthlyRanking(visible.getId(), baseDate, 2, 10.0);
        ...
        @Test
        @DisplayName("date 파라미터 생략 시 어제 날짜 기준 월간 랭킹을 조회한다.")
        void defaultToYesterday() {
            // given
            Brand brand = saveBrand();
            Product product = saveProduct(brand, "상품", "Y");
            LocalDate yesterday = LocalDate.now(clock).minusDays(1);
            seedMonthlyRanking(product.getId(), yesterday, 1, 10.0);
        ...
        void pagination() {
            ...
            LocalDate baseDate = LocalDate.now(clock).minusDays(1);
            seedMonthlyRanking(p1.getId(), baseDate, 1, 30.0);
            seedMonthlyRanking(p2.getId(), baseDate, 2, 20.0);
            seedMonthlyRanking(p3.getId(), baseDate, 3, 10.0);
