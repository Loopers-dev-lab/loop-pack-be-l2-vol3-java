...
class ProductStockRepositoryImplTest {
    ...
    private ProductStockModel createStock(String productId, int onHand, int reserved) {
    ...
    void reserveStock_CAS_WithSufficientStock_ShouldReturnAffectedRows1() {
        createStock("product-1", 100, 0);
        entityManager.flush();
        entityManager.clear();

        int affected = stockRepository.reserveStock("product-1", 50);

        assertThat(affected).isEqualTo(1);
        ProductStockModel stock = stockRepository.findByProductId("product-1").get();
    ...
    void reserveStock_CAS_WithInsufficientStock_ShouldReturnAffectedRows0() {
        createStock("product-1", 10, 5);
        entityManager.flush();
        entityManager.clear();

        int affected = stockRepository.reserveStock("product-1", 10);
    ...
    void releaseStock_CAS_ShouldDecreaseReserved() {
        createStock("product-1", 100, 50);
        entityManager.flush();
        entityManager.clear();

        int affected = stockRepository.releaseStock("product-1", 30);
    ...
    void commitStock_CAS_ShouldDecreaseBothOnHandAndReserved() {
        ...
        int affected = stockRepository.commitStock("product-1", 30);
