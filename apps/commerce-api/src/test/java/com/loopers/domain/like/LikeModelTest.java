...
class LikeModelTest {
    ...
    void create_WithValidInputs_ShouldSuccess() {
        LikeModel like = LikeModel.create("user-001", "product-001");

        assertThat(like.getUserId()).isEqualTo("user-001");
        assertThat(like.getProductId()).isEqualTo("product-001");
    }

    @Test
    @DisplayName("userId가 null이면 CoreException 발생")
    void create_WithNullUserId_ShouldThrow() {
        assertThatThrownBy(() -> LikeModel.create(null, "product-001"))
    ...
    void create_WithNullProductId_ShouldThrow() {
        assertThatThrownBy(() -> LikeModel.create("user-001", null))
                .isInstanceOf(CoreException.class);
    }
