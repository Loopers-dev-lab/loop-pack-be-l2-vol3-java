 ...
 * 장바구니 Facade (퍼사드)
 *
 * <p>UserService, CartService, ProductService, StockService, BrandService를
...
public class CartFacade {
     ...
     */
    public List<CartInfo> getCart(String loginId, String loginPw) {
     ...
     * @return 해당 사용자의 장바구니 항목 목록 (상품/브랜드/재고 정보 포함)
     */
    public List<CartInfo> getCartForAdmin(String userId) {
    ...
    private List<CartInfo> buildCartInfoList(List<CartItemModel> items) {
        ...
        Map<String, ProductModel> productMap = productService.findAllByIds(productIds)
                .stream().collect(Collectors.toMap(ProductModel::getProductId, Function.identity()));

        Set<String> brandIds = productMap.values().stream()
                .map(ProductModel::getBrandId).collect(Collectors.toSet());
        Map<String, BrandModel> brandMap = brandService.findAllByIds(brandIds)
                .stream().collect(Collectors.toMap(BrandModel::getBrandId, Function.identity()));

        Map<String, ProductStockModel> stockMap = stockService.findAllByProductIds(productIds)
