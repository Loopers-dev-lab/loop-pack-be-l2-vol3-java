...
public interface CartV1ApiSpec {
    ...
    ApiResponse<Object> addToCart(User user, CartV1Dto.AddRequest request);

    @Operation(summary = "장바구니 조회", description = "내 장바구니를 조회합니다.")
    ApiResponse<CartV1Dto.CartResponse> getMyCart(User user);

    @Operation(summary = "수량 변경", description = "장바구니 항목의 수량을 변경합니다.")
    ApiResponse<Object> updateQuantity(User user, Long cartItemId, CartV1Dto.UpdateQuantityRequest request);

    @Operation(summary = "항목 삭제", description = "장바구니에서 항목을 삭제합니다.")
    ApiResponse<Object> removeItem(User user, Long cartItemId);
}
