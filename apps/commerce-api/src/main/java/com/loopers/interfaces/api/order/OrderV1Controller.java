 ...
 * 여러 서비스(주문, 재고, 장바구니 등)를 조합하여 처리한다.
 * 모든 처리는 {@link OrderFacade}를 통해 위임한다.</p>
 */
@RestController
...
public class OrderV1Controller {
    ...
    public ResponseEntity<ApiResponse<OrderV1Dto.OrderDetailResponse>> createDirectOrder(
        ...
        OrderInfo info = orderFacade.createDirectOrder(user.getUserId(), request.toItems(), request.getCouponId());
    ...
    public ResponseEntity<ApiResponse<OrderV1Dto.OrderDetailResponse>> createCartOrder(
        ...
        OrderInfo info = orderFacade.createCartOrder(user.getUserId(), request.toItems(), request.getCouponId());
        return ResponseEntity.status(201).body(ApiResponse.success(OrderV1Dto.OrderDetailResponse.from(info)));
    }
    ...
    public ResponseEntity<ApiResponse<List<OrderV1Dto.OrderResponse>>> getOrders(
        ...
        List<OrderV1Dto.OrderResponse> response = orderFacade.getOrders(user.getUserId(), start, end).stream()
    ...
    public ResponseEntity<ApiResponse<OrderV1Dto.OrderDetailResponse>> getOrderDetail(
        ...
        OrderInfo info = orderFacade.getOrderDetail(user.getUserId(), orderId);
    ...
    public ResponseEntity<ApiResponse<Object>> cancelOrder(
            @AuthUser UserModel user,
            @PathVariable Long orderId) {
        orderFacade.cancelOrder(user.getUserId(), orderId);
