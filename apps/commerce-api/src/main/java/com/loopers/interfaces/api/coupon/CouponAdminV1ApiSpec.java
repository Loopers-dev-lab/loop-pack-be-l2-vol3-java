...
public interface CouponAdminV1ApiSpec {
    ...
    @Operation(summary = "쿠폰 템플릿 목록 조회", description = "관리자가 쿠폰 템플릿 목록을 페이지 단위로 조회합니다.")
    ApiResponse<CouponAdminV1Dto.CouponTemplateListResponse> getTemplates(int page, int size);

    @Operation(summary = "쿠폰 템플릿 상세 조회", description = "관리자가 특정 쿠폰 템플릿의 상세 정보를 조회합니다.")
    ...
    @Operation(summary = "쿠폰 템플릿 삭제", description = "관리자가 쿠폰 템플릿을 삭제합니다. (발급 쿠폰도 함께 soft delete)")
    ApiResponse<Void> deleteTemplate(Long couponId);

    @Operation(summary = "쿠폰 발급 내역 조회", description = "관리자가 특정 쿠폰 템플릿의 발급 내역을 조회합니다.")
