...
public interface QueueV1ApiSpec {
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "X-USER-ID 헤더 누락"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "429", description = "대기열이 가득 참")
    })
    ResponseEntity<ApiResponse<EnterResponse>> enterQueue(
            @Parameter(description = "회원 ID") @RequestHeader("X-USER-ID") Long memberId
    );
