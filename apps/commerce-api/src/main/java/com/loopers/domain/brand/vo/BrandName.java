...
public record BrandName(String value) {
    ...
    private void validate(String name) {
        if (name == null || name.isBlank()) {
            throw new BrandValidationException("브랜드 이름은 필수입니다.");
        }
        if (name.length() > MAX_LENGTH) {
            throw new BrandValidationException("브랜드 이름은 " + MAX_LENGTH + "자를 초과할 수 없습니다.");
        }
        if (!VALID_PATTERN.matcher(name).matches()) {
            throw new BrandValidationException("브랜드 이름에는 영문, 한글, 숫자, '-', '_', '.'만 사용할 수 있습니다.");
        }
    }
}
