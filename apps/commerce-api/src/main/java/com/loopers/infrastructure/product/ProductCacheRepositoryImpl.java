...
public class ProductCacheRepositoryImpl implements ProductCacheRepository {
    ...
    private static final String LIST_CACHE_KEY_PATTERN = "loopers:product:list:*";
    private static final String DETAIL_KEY_PREFIX     = "loopers:product:detail:";
    ...
    public Optional<ProductInfo> getDetail(Long productId) {
        try {
            String json = readTemplate.opsForValue().get(DETAIL_KEY_PREFIX + productId);
            if (json == null) {
                return Optional.empty();
            }
            return Optional.of(objectMapper.readValue(json, ProductInfo.class));
        } catch (Exception e) {
            log.warn("상품 상세 캐시 조회 실패 (productId={}): {}", productId, e.getMessage());
            return Optional.empty();
        }
    }

    @Override
    public void saveDetail(Long productId, ProductInfo info) {
        try {
            String json = objectMapper.writeValueAsString(info);
            long ttlSeconds = cacheProperties.getTtlSeconds("product:detail");
            writeTemplate.opsForValue().set(DETAIL_KEY_PREFIX + productId, json, Duration.ofSeconds(ttlSeconds));
        } catch (Exception e) {
            log.warn("상품 상세 캐시 저장 실패 (productId={}): {}", productId, e.getMessage());
        }
    }
    ...
    public void evictDetail(Long productId) {
        try {
            writeTemplate.delete(DETAIL_KEY_PREFIX + productId);
        } catch (Exception e) {
            log.warn("상품 상세 캐시 무효화 실패 (productId={}): {}", productId, e.getMessage());
        }
    }
}
