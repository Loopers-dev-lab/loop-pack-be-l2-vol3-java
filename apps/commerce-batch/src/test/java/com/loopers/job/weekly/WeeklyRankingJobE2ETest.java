package com.loopers.job.weekly;
...
class WeeklyRankingJobE2ETest {
    ...
    void completedWithEmptyMetrics() throws Exception {
        ...
        assertAll(
                ...
                () -> assertThat(countByBaseDate("mv_product_rank_weekly", targetDate.minusDays(1))).isZero()
        );
    }
    ...
    void populatesMvTableWithRanking() throws Exception {
        ...
        LocalDateTime bucket = targetDate.minusDays(3).atTime(12, 0); // 윈도우 내
        ...
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT product_id, `rank`, score FROM mv_product_rank_weekly WHERE base_date = ? ORDER BY `rank`",
                baseDate
        );
        assertAll(
                () -> assertThat(jobExecution.getExitStatus().getExitCode())
    ...
    void replacesExistingMvOnRerun() throws Exception {
        ...
        LocalDateTime bucket = targetDate.minusDays(3).atTime(12, 0);
        ...
        LocalDate baseDate = targetDate.minusDays(1);
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT product_id, `rank` FROM mv_product_rank_weekly WHERE base_date = ? ORDER BY `rank`",
                baseDate
        );
        assertAll(
                () -> assertThat(jobExecution.getExitStatus().getExitCode())
    ...
    void excludesDataOutsideWindow() throws Exception {
        // given — 윈도우: [targetDate-7일 00:00, targetDate 00:00)
        LocalDate targetDate = LocalDate.of(2026, 4, 20);
        LocalDateTime outsideWindow = targetDate.minusDays(8).atTime(12, 0); // 윈도우 시작보다 하루 이전
        LocalDateTime insideWindow = targetDate.minusDays(6).atTime(12, 0);  // 윈도우 내
    ...
    void aggregatesMultipleBucketHoursForSameProduct() throws Exception {
        ...
        LocalDateTime bucket1 = targetDate.minusDays(3).atTime(10, 0);
        LocalDateTime bucket2 = targetDate.minusDays(3).atTime(11, 0);
