package com.loopers.job.weekly.step;
...
@DisplayName("WeeklyRankingItemWriter 단위 테스트")
class WeeklyRankingItemWriterTest {

    private MvProductRankRepository mvProductRankRepository;
    private WeeklyRankingItemWriter writer;

    @BeforeEach
    void setUp() {
        mvProductRankRepository = mock(MvProductRankRepository.class);
        writer = new WeeklyRankingItemWriter(mvProductRankRepository);
        ReflectionTestUtils.setField(writer, "targetDate", LocalDate.of(2026, 4, 11));
    }

    @Test
    @DisplayName("write() 가 두 번 호출되면 rank 가 연속되고 replaceWeeklyRanking 은 afterStep() 에서 1회만 호출된다")
    void multipleWriteCallsProducesContinuousRanks() throws Exception {
        ...
        verify(mvProductRankRepository, times(1))
                .replaceWeeklyRanking(org.mockito.ArgumentMatchers.eq(LocalDate.of(2026, 4, 10)), captor.capture());
