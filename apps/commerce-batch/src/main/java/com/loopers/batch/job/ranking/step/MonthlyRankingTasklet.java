 ...
 * 직전 30일(당일 제외)의 {@code product_metrics}를 집계하여 상품별 가중 스코어 상위 100개를 {@code mv_product_rank_monthly}에 upsert한다.</p>
 *
 * <p>스코어 공식: {@code view_count × 0.1 + like_count × 0.2 + order_count × 0.7}</p>
 *
 * <p>집계 범위 예시 (date=20260414):</p>
 * <pre>
...
public class MonthlyRankingTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        ...
        LocalDateTime start = baseDate.minusDays(30).atStartOfDay();
        ...
        List<ProductRankingMonthly> rankings = topScores.stream()
