...
/**
 * 주간 집계 Reader Factory. {@code product_metrics_daily}를 {@code GROUP BY product_id}로
 * 집계하여 Chunk 단위로 읽어 Processor에 전달한다.
 *
 * <p>집계 윈도우: {@code [baseDate - 6, baseDate]} (롤링 7일, 양 끝 포함).</p>
 */
public final class WeeklyProductMetricsReader {

    private static final int PAGE_SIZE = 1000;
    private static final int WINDOW_DAYS = 7;

    private WeeklyProductMetricsReader() {
    }

    public static JdbcPagingItemReader<ProductAggregate> create(DataSource dataSource, LocalDate baseDate) {
        LocalDate from = baseDate.minusDays(WINDOW_DAYS - 1L);
