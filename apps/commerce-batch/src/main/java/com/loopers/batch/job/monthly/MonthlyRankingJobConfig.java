 ...
 *
 * 실행 조건:
 *   spring.batch.job.name=monthlyRankingJob 일 때만 Bean 이 활성화된다.
 *
 * 필수 JobParameter:
 *   targetDate (LocalDate) — 집계 기준 상한일. 슬라이딩 윈도우는 [targetDate-30, targetDate) 이다.
 *
 * 처리 흐름:
 *   1. Reader: product_metrics_hourly 를 30일 윈도우로 집계하여 score DESC 100건 커서 조회
 *   2. Writer: Chunk 순서(=rank) + targetDate-1 의 base_date 로 mv_product_rank_monthly 에 저장
 *
 * WeeklyRankingJobConfig 와 구조가 동일하며, 집계 윈도우(30일)와 대상 테이블만 다르다.
 */
...
public class MonthlyRankingJobConfig {
     ...
     * MonthlyRankingItemWriter 의 rank 할당(1부터 순서대로)이 올바르게 동작한다.
     */
     ...
     *   4: startTime   (targetDate - 30일 00:00, 포함)
    ...
    public JdbcCursorItemReader<ProductMetricsAggregate> monthlyRankingReader(
            DataSource dataSource,
            RankingWeights rankingWeights,
            @Value("#{jobParameters['targetDate']}") LocalDate targetDate
    ) {
        // 슬라이딩 윈도우: targetDate 기준 직전 30일
        LocalDateTime startTime = targetDate.minusDays(30).atStartOfDay();
