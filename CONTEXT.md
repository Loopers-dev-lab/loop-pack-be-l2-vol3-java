...
- [x] **스케줄러 원자성 문제**: ZRANGE → ZREM → SET 3단계가 분리되어 있음 ✅
...
- [x] **토큰 보유 중 대기열 재진입**: 미구현 (이상 상태 허용) ✅
  - 고민: 토큰 받은 유저가 주문 안 하고 다시 대기열 진입 시도 → 토큰 + 대기열 동시 존재 이상 상태
  - 선택: 미구현. ZADD NX로 등록은 되나, Lua 스크립트에서 EXISTS == 1이면 스킵 → 토큰 TTL(300초) 만료 전까지 대기열에서 제거 안 됨
  - 트레이드오프: 해당 유저가 순번을 차지한 채 방치될 수 있음. 이탈 감지 로직이 없어 300초 동안 순번 왜곡 가능
  - 미구현 이유: 이탈 감지 전체가 시뮬레이션 후 결정으로 남아있어 이번 구현 범위에서 제외

- [x] **스케줄러 실행 시간 > 주기**: Redisson 분산 락으로 방지 ✅
...
- [x] **Polling 주기**: Adaptive Polling 구현 ✅
  ...
  - 구체적 수치: 순번 ≤10 → 5초, ≤50 → 15초, >50 → 30초 (시뮬레이션 전 초기값, 추후 조정 가능)
...
| 스케줄러 원자성 | Lua 스크립트 | ZRANGE+EXISTS+ZREM 3단계 원자적 처리, 유령 상태 방지 |
| 토큰 값 | UUID | userId 알아도 위조 불가, 존재 여부가 아닌 값 일치로 검증 |
| Adaptive Polling 수치 | ≤10 → 5초, ≤50 → 15초, >50 → 30초 | 초기값, 시뮬레이션 후 조정 가능 |
