...
`docs/plan`을 기반으로 구현할 때 아래 절차를 **반드시** 따른다.

### 선택사항 발생 시
구현 중 설계 선택이 필요한 순간이 생기면:

1. **대안을 plan 파일에 먼저 기록**한다.
   - 위치: `docs/plan/{주차}/{브랜치명}-plan.md` 해당 커밋 단위 항목 아래
   - 형식:
     ```
     > 선택사항: [주제]
     > A: [방법명] — 방식 / 장점 / 단점
     > B: [방법명] — 방식 / 장점 / 단점
     ```
2. **개발자에게 선택을 요청**하고 응답을 기다린다.
3. 개발자가 선택하면 **plan 파일에 결정 내용을 추가**한다.
   ```
   > 결정: [선택된 방법] — [이유]
   ```
4. **discussion 파일에도 동일 내용을 기록**한다. (아래 과정 기록 규칙 적용)

### Discussion 기록 의무
- 설계 선택이 발생한 **모든 경우** discussion에 기록한다.
- 구현 완료 후 기록하지 않는 것은 **규칙 위반**이다.
- discussion 기록 없이 다음 단계로 진행 금지.

### 의사결정 후 하위 계획 연쇄 반영 의무
개발자가 선택을 완료하면, 해당 결정이 영향을 미치는 **하위 커밋 단위 항목들을 plan 파일에서 즉시 검토하고 수정**한다.

1. plan 파일의 이후 항목 중 결정에 의해 바뀌어야 할 내용(파일명, 구현 방식, 의존 관계 등)을 찾는다.
2. 변경이 필요한 항목을 모두 수정한 뒤 개발자에게 변경 사항을 요약 보고한다.
3. 변경이 없더라도 "하위 항목 검토 완료, 영향 없음"을 명시한다.

- ❌ 결정된 사항을 plan 현재 항목에만 반영하고 이후 항목은 그대로 두는 행위
- ❌ 하위 항목 검토 없이 바로 구현 시작하는 행위
- ❌ 선택사항을 plan에 기록하지 않고 구두로만 질문하는 행위
- ❌ 개발자 선택 후 discussion에 기록하지 않고 바로 구현 시작하는 행위
- ❌ 구현 완료 후 discussion 기록을 생략하는 행위
...
- `/technical-writing` — 실험 기반 테크니컬 블로그 글 작성 (가설→실험→반전 서사, Mermaid, 표 중심)
