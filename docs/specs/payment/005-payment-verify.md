...
## 배경
PG 타임아웃으로 미확정(REQUESTED) 상태인 결제에 대해 사용자가 수동으로 PG에 상태를 확인하여 결제를 확정할 수 있어야 한다.
...
- [ ] 이미 확정(SUCCEEDED/FAILED/CANCELED) 또는 취소 진행 중(CANCEL_REQUESTED) 상태이면 400 응답, 메시지: "이미 확정되었거나 취소 진행 중인 결제입니다"
