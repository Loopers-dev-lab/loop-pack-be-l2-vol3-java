...
| 항목 | 저장 여부 | 이유 |
|------|----------|------|
| product_name | O | 주문 내역에 반드시 필요 |
| product_price | O | 결제 금액 증빙 (법적 필수) |
| brand_name | O | 주문 상세에서 브랜드 표시 필요 |
| image_url | X | 표시용, 없어도 무방 (placeholder 처리) |
| description | X | 주문 내역에 불필요 |

**원칙**: 스냅샷은 "그 순간을 재현할 수 있어야 한다"
