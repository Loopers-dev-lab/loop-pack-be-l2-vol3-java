...
- **시스템 관점**: 주문/취소 트랜잭션과 결제 연동을 분리해, 트랜잭션 커밋 이후 비동기 결제 요청/취소 및 콜백 기반 재고 차감을 지원해야 한다.
...
**수용 기준 (AC):**
- [ ] AC1: Given 인증 + 모든 상품 활성 + 재고 충분, When POST /api/v1/orders, Then 201 + `(쿠폰 차감 -> 포인트 차감 -> 주문 생성)` 수행 + 커밋 후 비동기 결제 요청
...
- 주문 트랜잭션 순서: `쿠폰 차감 -> 포인트 차감 -> 주문 생성`
- 결제 요청은 주문 트랜잭션 커밋 이후 비동기로 수행
- 재고 차감은 결제 콜백(또는 상태 수렴)에서 `SUCCEEDED` 확인 후 수행
...
**수용 기준 (AC):**
- [ ] AC1: Given 인증 + 본인의 ORDERED 상태 주문, When PATCH /api/v1/orders/{orderId}/cancel, Then 200 + `쿠폰 복구 -> 포인트 복구 -> 재고 복구 -> 주문 소프트 삭제` + 커밋 후 비동기 결제 취소
...
**수용 기준 (AC):**
- [ ] AC1: Given LDAP 인증 + ORDERED 상태 주문, When PATCH /api-admin/v1/orders/{orderId}/cancel, Then 200 + `쿠폰 복구 -> 포인트 복구 -> 재고 복구 -> 주문 소프트 삭제` + 커밋 후 비동기 결제 취소
...
- 주문 유스케이스에서 쿠폰, 포인트, 주문 생성은 하나의 비즈니스 트랜잭션으로 다뤄져야 하며 부분 성공이 없어야 한다.
- 동시성 제어는 도메인 특성에 따라 낙관적 락/비관적 락 중 선택하며, 선택 근거를 문서화한다.
- 쿠폰 단일 사용 보장은 DB 제약조건(유니크/상태 전이 조건)과 애플리케이션 검증을 함께 사용한다.
- 트랜잭션 실패 시 쿠폰 상태/포인트/주문 데이터는 원자적으로 롤백되어야 한다.
...
- 최소 주문 금액 조건(minOrderAmount)은 주문 상품들의 `가격 * 수량` 합계 기준으로 검증한다
