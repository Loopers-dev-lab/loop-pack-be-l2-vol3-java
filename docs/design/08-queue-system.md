...
- [ ] EntryTokenInterceptor에 로컬 Rate Limit fallback 추가 — Redis 장애 시 주문 전면 차단 방지
- [ ] Burst Traffic 부하 테스트 (10만명/1초 시나리오) — 대기열 한계 48,000명 검증
- [ ] Sustained Load 부하 테스트 (80% TPS 장시간) — steady state 유지 확인
