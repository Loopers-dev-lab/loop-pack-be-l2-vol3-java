...
classDiagram
    ...
    class User {
        ...
        +changePassword(currentPw, newPw) User
    }
...
- **불변 도메인 객체**: 기존 User가 record 기반 불변 객체 + Value Object(UserId, Password, Name, Email, BirthDate) 패턴으로 구현되어 있다. 새 도메인도 동일 패턴 적용.
...
- **Like = 조인 엔티티**: User-Product 간 N:M 관계를 Like 엔티티로 풀어낸다. DB에서 (userId + productId) Unique 제약조건으로 중복 방지.
