...
```jsx
[유저] → 대기열 진입 (POST /api/v1/queue/enter)
      → 대기열에서 순번 부여
      → 순번 조회 (GET /api/v1/queue/position 또는 SSE /api/v1/queue/position/stream)
      → 스케줄러가 방출 후 입장 토큰 발급 (Redis SET+TTL)
      → 토큰으로 주문 API 호출 (POST /api/v1/orders + X-Entry-Token)
      → placeOrder 초입에서 토큰 Lua 소비 → 주문 처리
```
...
```jsx
SET   queue:entry:{userId}  {token}  EX 300   // 5분 TTL (queue.scheduler.token-ttl-seconds)
// 주문: Lua consumeIfTokenMatches — 일치 시 DEL (placeOrder 본문 전)
...
```jsx
  [클라이언트]
     └── 권장 간격은 응답의 suggestedPollIntervalMs / Retry-After (QueuePollHintPolicy)
...
```jsx
[클라이언트] → GET /api/v1/queue/position/stream (SSE, 동시 연결 상한 있음)
          ← event: position
...
구현에서는 스케줄러가 토큰 `SET` 직전 **0~300ms** 지연(`EntrySchedulerService`)으로 Redis 쓰기 스파이크를 줄입니다. (교육용으로 자주 쓰는 “0~2초 활성화 지연” 설명과는 별개·축소 구현입니다.)
...
- 토큰을 받고 주문하지 않으면 TTL로 만료됩니다. 문서상 “틱마다 만료 추정치로 배치 보정”하는 고급 정책은 **현재 코드에 없음**(고정 max-batch-size).
...
[유저] → GET /api/v1/queue/position (suggestedPollIntervalMs / Retry-After 준수)
      → 현재 순번 + 예상 대기 시간 응답
...
[유저] → 순번 0 도달, 토큰 수신
      ...
      → 토큰 검증·Lua 소비(placeOrder 초입) → 주문 처리
