...
### 3.5 쿠폰 낙관락 재시도·백오프 기준

재시도 횟수와 백오프 시간 선택의 근거를 명시한다. 구현은 CouponService.validateAndUse(재시도 루프) + validateAndUseInNewTransaction(REQUIRES_NEW)에서 적용한다.

| 항목 | 채택값 | 기준 |
|------|--------|------|
| **재시도 횟수** | **최대 2회 시도**(재시도 1회) | 동시에 같은 쿠폰을 쓰는 요청이 2~3건 수준이면 1회 재시도로 한 건은 성공할 가능성이 높음. 2회 연속 낙관락 실패 시에는 "선점 실패"로 보고 409로 빠르게 종료. 3회 이상 재시도는 비권장 — 경합이 심한 상황으로 보고 클라이언트 재요청("잠시 후 다시 시도") 유도가 적절함. |
| **백오프 시간** | **50ms 고정** | 다른 트랜잭션의 커밋/롤백이 끝날 시간을 주어 재시도 시 충돌 확률을 낮춤. 30ms 미만은 경합 해소 효과가 작고, 100ms 이상이면 사용자 체감 지연이 커짐. 30~100ms 범위 내에서 고정값 선택. |
| **실패 시 응답** | **409 CONFLICT** + "잠시 후 다시 시도해 주세요" | 무한 재시도 방지, 클라이언트에 재시도 유도 메시지로 일관된 정책 전달. |

**검증·모니터링(선택)**  
- 쿠폰 사용 API의 409 비율·평균/P99 응답 시간을 지표로 둔다.  
- 409 비율이 일정 수준(예: 5%) 이상이면 재시도 횟수·백오프 재검토 또는 비관적 락 전환을 검토한다.  
- 동일 쿠폰 동시 요청 N건(예: 5~10) 부하 테스트로 "2회 시도·50ms"가 목표(1건 성공·나머지 409)에 맞는지 확인한다.

### 3.6 DB 커넥션 유지 시간 (Connection Starvation)
...
| 1-1 | **Connection Starvation** | 쿠폰 조회 → 재고 락 순서에서 재고 락 대기 시 커넥션 점유. **Lock Timeout 짧게 설정 필수 병행.** (§3.6) |
